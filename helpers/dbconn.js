...
const daftar = async (id,namaPel,domPel,noPel) => {
	...
	if(rows.length == 0) {
		const a = await connection.query('INSERT INTO m_member (memberid,membername,joindate,address3,hp,race,user_id,membertypeid,changesdate,create_date,mme_createdby,mme_createddt,mme_updatedby,mme_updateddt) VALUES (?,?,now(),?,?,?,?,?,now(),now(),?,now(),?,now() + INTERVAL 10 MINUTE)', [id,namaPel,domPel,noPel,1,"admin","S1","admin","admin"]);
...
const update = async (namaPel,domPel,noPel) => {
	...
	const [rows] = await connection.query('SELECT memberid FROM m_member WHERE hp = ?', [noPel]);
	if(rows.length > 0) {
		const a = await connection.query('UPDATE m_member set membername =? ,address3= ?,changesdate=now(),mme_createddt=now(),mme_updateddt=now()+ INTERVAL 10 MINUTE WHERE hp=?', [namaPel,domPel,noPel]);
